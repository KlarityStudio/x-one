...
class AW_Admin_Controller_Reports extends AW_Admin_Controller_Abstract
{
	...
	{
		$current_tab = self::get_current_tab();
		$current_tab->handle_actions( self::get_current_route() );
	}
	...
	/**
	 * @return AW_Admin_Reports_Tab_Abstract
	 */
	...
	static function get_reports_tabs()
	{
		if ( empty( self::$reports ) )
		{
			...
			{
				$report_includes[] = $path . 'carts.php';
			}

			$report_includes[] = $path . 'guests.php';
			$report_includes[] = $path . 'queue.php';
			$report_includes[] = $path . 'unsubscribes.php';
