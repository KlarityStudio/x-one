...
abstract class AW_Model
{
	...
	{
		if ( ! $this->get_meta_table_name() ) return false;

		// check meta cache
		if ( isset( $this->meta_cache[$key] ) ) return $this->meta_cache[$key];
	...
	private function _can_add_meta()
	{
		...
		if ( ! $this->get_meta_table_name() || ! $this->meta_id_column ) return false;
