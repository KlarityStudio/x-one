...
class AW_Email
{
	...
	function catch_unsubscribe_url()
	{
		...
		$user_email = is_email( aw_request( 'user' ) );
		...
		if ( aw_request('confirmed') )
		{
			...
			if ( $workflow_id && $user_email )
			{
				...
				if ( $workflow->exists && $user instanceof WP_User )
				{
					if ( $workflow->unsubscribe_user( $user->ID ) )
					{
						$notice_type = 'success';
						$success = true;
		...
		else
		{
			...
			aw_get_template(
				...
				array(
					'unsubscribe_confirm_url' => $this->generate_unsubscribe_url( $workflow_id, $user_email, true )
				)
			);
		}
