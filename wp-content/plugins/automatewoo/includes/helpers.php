...
{
	if ( ! AW_Url_Shortener::check() ) return $content;

	$replacer = new AW_Replace_Helper( $content, array( 'AW_Url_Shortener', 'process' ), 'text_urls' );
...
function aw_display_date( $timestamp, $max_diff = false, $convert_from_gmt = true )
{
	...
	{
		return $date_to_display = date_i18n( wc_date_format(), $timestamp );
	}
...
function aw_display_time( $timestamp, $max_diff = false, $convert_from_gmt = true )
{
	...
	if ( abs( $diff ) > $max_diff )
	{
		return $date_to_display = date_i18n( wc_date_format() . ' \a\t ' . wc_time_format(), $timestamp );
	}
