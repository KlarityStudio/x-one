...
class AW_Model_Log extends AW_Model
{
	...
	function has_click_recorded()
	{
		...
		if ( is_array( $tracking ) ) foreach( $tracking as $item )
		{
			if ( $item['type'] == 'click') return true;
		}
	...
	function get_date_opened()
	{
		...
		if ( is_array( $tracking ) ) foreach( $tracking as $item )
		{
			if ( $item['type'] == 'open') return $item['date'];
		}
	...
	function get_date_clicked()
	{
		...
		if ( is_array( $tracking ) ) foreach( $tracking as $item )
		{
			if ( $item['type'] == 'click') return $item['date'];
		}
	...
	{
		if ( ! $workflow = $this->get_workflow() ) return; // workflow must be set

		if ( ! $this->exists ) return; // log must be saved
