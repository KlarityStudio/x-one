...
class Easy_User_Tags
{
	...
	function __construct()
	{
		...
		add_filter( 'restrict_manage_users', array( $this, 'inject_bulk_actions' ), 1, 1 );
	...
	function user_profile($user)
	{
		...
		foreach(self::$taxonomies as $key=>$taxonomy):
			...
			<table class="form-table">
				<tr>
					<th><label for=""><?php printf(__("Select %s", 'easy-user-tags'), $taxonomy->labels->name ) ?></label></th>
					<td>
						<?php if(!empty($terms)):?>
						...
						<?php else:?>
							<?php printf(__("There are no %s available.", 'easy-user-tags'), $taxonomy->labels->name ) ?>
						<?php endif?>
					</td>
				</tr>
			</table>
	 ...
	 *
	 */
	function inject_bulk_actions()
	{
		if ( current_user_can( 'edit_users' ) ) : ?>

			<label class="screen-reader-text" for="new_role"><?php _e( 'Add tag&hellip;', 'easy-user-tags' ) ?></label>
			<select name="add_user_tag" id="add_user_tag">
				<option value=""><?php _e( 'Add tag', 'easy-user-tags' ) ?></option>
				<?php wp_dropdown_user_tags(); ?>
			</select>

			<label class="screen-reader-text" for="new_role"><?php _e( 'Remove tag&hellip;', 'easy-user-tags' ) ?></label>
			<select name="remove_user_tag" id="remove_user_tag">
				<option value=""><?php _e( 'Remove tag', 'easy-user-tags' ) ?></option>
				<?php wp_dropdown_user_tags(); ?>
			</select>
