...
class AW_Mailer
{
	...
	function send()
	{
		...
		if ( $this->user_is_unsubscribed() )
		{
			return new WP_Error( 2, __( 'The user is unsubscribed from this workflow.', 'automatewoo' ) );
		}
		...
		if ( $sent === false )
		{
			return new WP_Error( 2, __( 'The wp_mail() function returned false.', 'automatewoo' ) );
		}
	...
	{
		if ( $this->user ) add_filter( 'woocommerce_email_footer_text', array( $this, '_email_footer_text' ) );
		...
		$this->get_template_part('email-header.php', array( 'email_heading' => $this->heading ) );
		...
		if ( $this->user ) remove_filter( 'woocommerce_email_footer_text', array( $this, '_email_footer_text' ) );
	 ...
	 * @return bool
	 */
	function user_is_unsubscribed()
	{
		if ( ! $this->user || ! $this->workflow )
			return false;

		return $this->workflow->is_user_unsubscribed( $this->user->ID );
	}
	 ...
	 * @return string
	 */
	function _email_footer_text( $text )
	{
		if ( empty( $this->user ) )
			return $text;
		...
		if ( $unsubscribe_url )
		{
			...
			$text .= sprintf(__('<a href="%s">'.$unsubscribe_text.'</a>'), $unsubscribe_url );
		}
	...
	{
		$tracking_pixel = '<img src="' . AW()->email->generate_open_track_url( $this->workflow->log->id ) . '" height="1" width="1" alt="" style="display:inline">';
